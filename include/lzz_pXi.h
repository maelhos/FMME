...
public:
    ...
    inline void SetLength(long n)
    {
        rep.SetLength(n);
    }

    inline zz_pXi<N-1>& operator[](long n)
    {
        return rep[n];
    }
...
string pp(const zz_pXi<N>& a)
{
    ...
    rec_mon<N, N>(ss, a, current_mon, p);
    return ss.str();
}
...
/*------------------------------------------------------------*/
template <size_t N>
void mul(zz_pXi<N>& c, const zz_pXi<N>& a, const zz_p& b){
    if (b == 0)
    {
        c.zero();
        return;
    ...
    for (long i = 0; i < dy; i++)
        mul(c.rep[i], c.rep[i], b);
    c.normalize();
}

template <>
inline void mul(zz_pXi<1>& c, const zz_pXi<1>& a, const zz_p& b){
    if (b == 0)
    {
        c.zero();
        return;
...
/*------------------------------------------------------------*/
template <size_t N>
void add(zz_pXi<N>& c, const zz_pXi<N>& a, const zz_p& b){
    if (b == 0)
        return;

    c = a;
    if (c.rep.length() == 0) c.rep.SetLength(1);
    add(c.rep[0], c.rep[0], b);
    c.normalize();
}

template <>
inline void add(zz_pXi<1>& c, const zz_pXi<1>& a, const zz_p& b){
    if (b == 0)
        return;
...
/*------------------------------------------------------------*/
template <size_t N>
zz_p _eval_impl(const zz_pXi<N>& c, const zz_p* point)
{
    zz_p ret(0);
    for (long d = c.degXn(); d >= 0; d--)
    {
        ret *= point[N - 1];
        ret += _eval_impl(c.rep[d], point);
    }
    return ret;
}

inline zz_p _eval_impl(const zz_pXi<1>& c, const zz_p* point)
{
    return eval(static_cast<zz_pX>(c), point[0]);
}

template <unsigned long N>
inline zz_p eval(const zz_pXi<N>& c, const array<zz_p, N>& point)
{
    return _eval_impl(c, point.data());
}
